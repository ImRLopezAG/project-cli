		...
		const project = await p.group(
			{
				...
				pgLite: ({ results }) => {
					if (results.databaseProvider === 'sqlite') {
						return undefined
					}
					return p.confirm({
						message:
							'Would you like to use pgLite (Electric SQL) for a serverless Postgres experience?',
						initialValue: false,
					})
				},
				apiLayer: ({ results }) => {
				...
				i18n: ({ results }) => {
					...
					return p.confirm({
						message:
							'Would you like to include internationalization (i18n) support with fbtee?',
						initialValue: false,
					})
				},
