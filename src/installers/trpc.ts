...
export const trpcInstallerDesktop: Installer = ({
	projectDir,
	packages,
	framework,
}) => {
	...
	const TRPC_PATHS = {
		SERVER: {
			INIT: {
				DEST: 'src/renderer/server/trpc/init.ts',
				WITH_AUTH: 'server/trpc/with-auth-init.ts',
				NO_AUTH: 'server/trpc/init.ts',
			},
			ROOT: {
				DEST: 'src/renderer/server/trpc/root.ts',
				WITH_AUTH: 'server/trpc/root.ts',
				NO_AUTH: 'server/trpc/root.ts',
			},
			ROUTES: {
				DEST: 'src/renderer/server/trpc/routes',
				WITH_AUTH: 'server/trpc/routes',
				NO_AUTH: 'server/trpc/routes',
			},
		},
		CLIENT: {
			DEST: 'src/renderer/lib/trpc',
			SRC: 'lib/trpc',
		},
		API_HANDLER: {
